<?php
	...
	add_action('factory_bootstrap_000_plugin_created', 'factory_bootstrap_000_plugin_created');
	function factory_bootstrap_000_plugin_created($plugin)
	{
		$manager = new FactoryBootstrap000_Manager($plugin);
	 ...
	 * @since 3.2.0
	 */
	class FactoryBootstrap000_Manager {
		...
		public function loadAssets($hook)
		{
			...
			if( !$compression ) {
				...
				foreach($this->scripts as $scriptToLoad) {
					wp_enqueue_script($scriptToLoad . '-' . $id, FACTORY_BOOTSTRAP_000_URL . "/assets/js/$scriptToLoad.js", $isFirst
						? $dependencies
				...
				foreach($this->styles as $styleToLoad) {
					wp_enqueue_style($styleToLoad . '-' . $id, FACTORY_BOOTSTRAP_000_URL . "/assets/flat/css/$styleToLoad.css", array(), $this->plugin->version);
				}
				// - //
				...
				if( !empty($this->styles) ) {
					$id = md5($loadStylesOut . FACTORY_BOOTSTRAP_000_VERSION);
					wp_enqueue_style('factory-bootstrap-000-' . $id, FACTORY_BOOTSTRAP_000_URL . '/includes/load-styles.php?folder=flat&load=' . $loadStylesOut, array(), $this->plugin->version);
				}

				if( !empty($this->scripts) ) {
					$id = md5($loadScriptsOut . FACTORY_BOOTSTRAP_000_VERSION);
					wp_enqueue_script('factory-bootstrap-000-' . $id, FACTORY_BOOTSTRAP_000_URL . '/includes/load-scripts.php?load=' . $loadScriptsOut, $dependencies, $this->plugin->version);
				}
			...
			if( $colorName !== 'fresh' ) {
				wp_enqueue_style('factory-bootstrap-000-colors', FACTORY_BOOTSTRAP_000_URL . '/assets/flat/css/bootstrap.' . $colorName . '.css');
			}
		...
		{
			if( substr($handle, 0, 22) !== 'factory-bootstrap-000-' ) {
				return $src;
			}
