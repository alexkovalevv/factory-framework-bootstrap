			...
			if ( file_exists( $cache_filepath ) ) {
				...
				foreach ( $sripts as $script_to_load ) {
					...
					if ( $cache_dir_exists ) {
						...
						if ( $type == 'js' ) {
							wp_enqueue_script( md5( $script_to_load ), FACTORY_BOOTSTRAP_000_URL . "/assets/$type-min/$script_to_load.min." . $type, $is_first ? $dependencies : false, $this->plugin->getPluginVersion() );
						} else {
							wp_enqueue_style( md5( $script_to_load ), FACTORY_BOOTSTRAP_000_URL . "/assets/$type-min/$script_to_load.min." . $type, [], $this->plugin->getPluginVersion() );
						}
						$is_first = false;
					}
				}
